...
class SCCoreore extends EventEmitter {
	async launch(options) {
		try {
			...
			if (!java.run) {
				this.emit(
					"debug",
					`[SCCore]: Couldn't start Minecraft due to: ${java.message}`
				);
				this.emit("close", 1);
				return null;
			}
			...
			if (this.options.installer) {
				...
				if (!this.options.version.custom && code === 0) {
					this.emit(
						"debug",
						"[SCCore]: Installer successfully ran, but no custom version was provided"
					);
				}
				this.emit("debug", `[SCCore]: Installer closed with code ${code}`);
			}
			...
			if (!fs.existsSync(mcPath)) {
				this.emit(
					"debug",
					"[SCCore]: Attempting to download Minecraft version jar"
				);
			...
			this.emit(
				"debug",
				`[SCCore]: Using ${separator} to separate class paths`
			);
			...
			this.emit("debug", "[SCCore]: Attempting to download assets");
			...
			this.emit(
				"debug",
				`[SCCore]: Launching with arguments ${launchArguments.join(" ")}`
			);

			return this.startMinecraft(launchArguments);
		} catch (e) {
			this.emit("debug", `[SCCore]: Failed to start due to ${e}, closing...`);
			return null;
		}
	}
	...
	printVersion() {
		if (fs.existsSync(path.join(__dirname, "..", "package.json"))) {
			...
			this.emit("debug", `[SCCore]: SCCore version ${version}`);
		} else {
			this.emit(
				"debug",
				"[SCCore]: Package JSON not found, skipping SCCore version check."
			);
		}
	}
	...
	createRootDirectory() {
		if (!fs.existsSync(this.options.root)) {
			this.emit("debug", "[SCCore]: Attempting to create root folder");
	...
	async extractPackage() {
		if (this.options.clientPackage) {
			this.emit(
				"debug",
				`[SCCore]: Extracting client package to ${this.options.root}`
			);
	...
	async getModifyJson() {
		...
		if (this.options.forge) {
			...
			this.emit(
				"debug",
				"[SCCore]: Detected Forge in options, getting dependencies"
			);
			modifyJson = await this.handler.getForgedWrapped();
		} else if (this.options.version.custom) {
			this.emit(
				"debug",
				"[SCCore]: Detected custom in options, setting custom version file"
			);
			modifyJson =
				modifyJson ||
				JSON.parse(
